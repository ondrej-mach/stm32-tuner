...
// ----------------- END OF USB CONFIGURATION GARBAGE ------------------------
...
int usb_init(void) {
	...
	// Set GPIOs 11, 12 to alternate function (USB data+, data-)
	gpio_mode_setup(GPIOA, GPIO_MODE_AF, GPIO_PUPD_NONE, GPIO11 | GPIO12);
	gpio_set_af(GPIOA, GPIO_AF10, GPIO11 | GPIO12);

	// VBUS from USB connector is not connected to PA9
    ...
    xTaskCreate(usb_task, "USB_POLL", 100, NULL, configMAX_PRIORITIES-1, NULL);
...
void usb_debug_log(char *str) {
    int len = MIN(strlen(str), 64);
	int wb;
